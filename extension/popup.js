...
async function waitForOPM(tabId, test, timeout = 30000) {
  ...
  while (Date.now() - started < timeout) {
    try { if (await run(tabId, test)) return true; } catch (_) {}
    await sleep(400);
  }
...
const typePostback = () => {
  ...
  // SES = M (per the OPM page's actual ddlTaskType options)
  el.value = 'M';
...
const typeReady = () => {
  ...
  return t instanceof HTMLSelectElement && t.value === 'M' && c instanceof HTMLSelectElement;
};
...
const selectTaskAndSave = async ticketId => {
  ...
  if (native instanceof HTMLSelectElement) {
    ...
    if (!(rendered instanceof HTMLElement) || !rendered.textContent.includes(ticketId)) throw new Error(`Task was not selected: ${ticketId}`);
  }
  ...
  $('start').addEventListener('click', async () => {
    ...
    try {
      for (let i = 0; i < rows.length; i++) {
        const t = rows[i];
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nSetting date...`); await run(tab.id, setDate, [date]);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nSelecting Raj ERP 4.0...`); await run(tab.id, projectPostback); await waitForOPM(tab.id, projectReady);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nSelecting SES...`); await run(tab.id, typePostback); await waitForOPM(tab.id, typeReady);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nFilling category and hours...`); await run(tab.id, fillCategoryAndHours, [t.hours]);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nSearching task...`); await run(tab.id, openTaskSearch, [t.id]);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nSelecting task and saving...`); await run(tab.id, selectTaskAndSave, [t.id]);
        status(`Ticket ${i + 1}/${rows.length}: ${t.id}\nWaiting for Save confirmation...`); await waitForOPM(tab.id, saveCompleted, 20000);
      }
